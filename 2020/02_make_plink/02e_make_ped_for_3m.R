...
pheno_3m <- read.csv("pheno_index_3m_2020.csv", head=T, stringsAsFactors=F, sep=";", fileEncoding="latin1")

# add unknown samples to pheno data

unknown_samples <- matrix(nrow=21, ncol=ncol(pheno_3m))

unknown_samples[,1] <- names(chip_data_list)[!(names(chip_data_list) %in% pheno_3m[,1])]

unknown_samples[,2:4] <- 0

unknown_samples[,5:11] <- NA 

colnames(unknown_samples) <- colnames(pheno_3m)

pheno_3m <- rbind(pheno_3m, unknown_samples)
...
for (n in (1:7)) {
	...
	x <- paste("ped_3m_", n, "_index.ped", sep="") 
