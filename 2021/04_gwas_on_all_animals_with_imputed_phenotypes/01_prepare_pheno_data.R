...
kin <- read.table("122_animals_with_pheno.aBN.kinf", head=F, stringsAsFactors=F)
...
pheno_qc_cov <- cbind(pheno_qc[,c(1:21, 29:31)], cov[,-1])

setwd("/home/common/projects/ovine_selection/Soft")

install.packages("phenix_1.0.tar.gz",type="source", repos=NULL)
library(phenix)

pheno_qc_imp <- phenix(Y=as.matrix(pheno_qc_cov[,-c(27:31)]), K=kin, scale=T)

pheno_qc_imp <- pheno_qc_imp$imp[,1:24]


# adjust phenotypes on covariates 

data <- cbind(cov[,-1], pheno_qc_imp)
...
pdf("pheno_imp_adj_122_animals_all.pdf")
...
# standartized adjust pheno

pheno_adj_st <- pheno_adj


for (n in 1:ncol(pheno_adj_st)) {

	pheno_adj_st[,n] <- pheno_adj[,n]/sd(pheno_adj[,n])

}


# count pc for all phenotypes

pheno_pc_adj <- princomp(pheno_adj_st)$scores

# compare cor (pc, mass_6d) and if cor<0 change pc to positive value

pheno_pc_adj_cor <- pheno_pc_adj

for (n in 1:ncol(pheno_pc_adj)) {

	if (cor(pheno_pc_adj[,n], pheno_adj_st[, "mass_6d"])<0) {

			pheno_pc_adj_cor[,n] <- - pheno_pc_adj[,n]
...
# normalized pc values

pheno_pc_adj_cor_norm <- matrix(ncol=ncol(pheno_pc_adj_cor), nrow=nrow(pheno_pc_adj_cor))
colnames(pheno_pc_adj_cor_norm) <- colnames(pheno_pc_adj_cor)
rownames(pheno_pc_adj_cor_norm) <- rownames(pheno_pc_adj_cor)

for (n in 1:ncol(pheno_adj)) {

	pheno_pc_adj_cor_norm[,n] <- qnorm((rank(pheno_pc_adj_cor[,n],na.last="keep")-0.5)/sum(!is.na(pheno_pc_adj_cor[,n])))
...
for_plot <- cbind(pheno_pc_adj_cor_norm, pheno_adj[,"mass_6d"])
colnames(for_plot)[25] <- "mass_6d"
for_plot <- cor(for_plot)

setwd("/home/common/projects/ovine_selection/GWAS_on_russian_population_of_ovines/2021/results/plots")

pdf("pheno_pc_norm_cor.pdf")
...
setwd("/home/common/projects/ovine_selection/GWAS_on_russian_population_of_ovines/2021/data/pheno_data/pheno_qc_122_animals/imputed")
...
setwd("/home/common/projects/ovine_selection/GWAS_on_russian_population_of_ovines/2021/data/pheno_data/pheno_qc_122_animals/pc")

for (n in (1:ncol(pheno_pc_adj_cor_norm))) {

	name_to_save <- paste("pc_", n, ".txt", sep="")

	write.table(pheno_pc_adj_cor_norm[,n], name_to_save, col.names=F, row.names=F, quote=F)
...
write.table(pheno_adj_norm, "pheno_imp_adj_norm_122_animals.txt", col.names=T, row.names=T, quote=F)